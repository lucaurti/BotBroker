...
namespace Broker.Common.Strategies.Momentum
{
    ...
    public class Strategy : IStrategy
    {
        ...
        void IStrategy.Events_onCandleUpdate(MyCandle myCandle)
        {
            ...
            MyMomentum oldMomentum = db.MyMomentums.OrderByDescending(s => s.Id).Skip(1).FirstOrDefault();
            decimal momentumValue = MyStrategy.MomentumAverage.Value();
            ...
            if (this.Current.PreviousAction == ActionType.WarmUp)
            {
                ...
                else this.Current.PreviousAction = ToActionType(Misc.GetParameterValue("lastOperation", "momentum"));
                ...
                this.Current.PreviousAction == ActionType.Buy)
            {
                ...
                if (MyStrategy.MACDAverage.isPrimed())
                {
                    ...
                    else
                    {
                        ...
                        Log.Debug("Current Momentum    : " + momentumValue.ToStringRound(2));
                        Log.Debug("Precedent Momentum  : " + oldMomentum.MomentumValue.ToStringRound(2));
        ...
        private void onEmitShort(long chatId, string text)
        {
            ...
            {                
                ...
                events.MyTradeRequest(settings, TradeAction.Short,
                    int.Parse(Misc.GetParameterValue("shortPercentage", "momentum")));
